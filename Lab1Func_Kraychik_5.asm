...
GF_Reciprocal PROC						; rcx - a (элемент поля)
	push rbx							; сохранение значения регистра rbx
	; rbx будет равен a^i, где i=0..63
	mov rbx, rcx
	m1:
		mov rcx, rbx					; умножаем rbx (текущее произведение)
		mov rdx, rcx					; на a
		call GF_MulX
		cmp rax, 1						; сравнение rax с единицей
		je m2							; если возвращенное значение равно 1, переход к m2
		mov rbx, rax					; обновление текущего произведения rbx
		jmp m1							; продолжить исполнение цикла
	m2:
	mov rax, rbx						; возвращаемое значение
